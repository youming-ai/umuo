...
- **File-based routes**: `[comp]/{index,news,stats,bracket,transactions,odds,teams}`, `[comp]/match/[slug]`, `[comp]/team/[id]`, `[comp]/player/[id]`, `api/[...route]`, `sitemap.xml.ts`. (`/scorers` 307-redirects to `/stats`; there is no global `/news` or top-level `/player`.)
...
### News feed
Per-competition news from ESPN's site.api league feed (`site/v2/.../{league}/news`, genuinely league-scoped — the old global "now" firehose was removed). `src/newsFeed.ts` (`parseNewsFeed` → `NewsItem[]`). Server side: `getCompNews` (api.ts; composes `serve(news)` + parse, empty array on any failure) seeds the `[comp]/news` page; `NewsIsland` + `NewsView` + `useNews.ts` take over client-side (120s visibility-gated poll, scroll-to-load). The `Ticker` island (module-level shared poller via `useTicker`, displays live/upcoming matches in a horizontal strip) is independent of the news feed.
