...
StreamCup is an **Astro 7 server-rendered app with React islands** for multiple sports competitions (English Premier League, NBA): a news-first competition hub, schedule/standings/scorers, ESPN-backed match detail pages, a cross-competition scores strip, and a global aggregated-news home. It deploys as a single Cloudflare Worker (Astro's SSR output) that renders pages and edge-caches the upstream data APIs in KV.

## Commands

```bash
bun run dev          # astro dev (port 4321). The @astrojs/cloudflare v14 adapter provisions a real local KV, so /api/* and SSR hit the actual cache path
...
Deploys to Cloudflare (config in `wrangler.jsonc`: Worker `main` = `@astrojs/cloudflare/entrypoints/server`, static `assets` from `dist/`, `CACHE` KV namespace). There is no `deploy` npm script — `wrangler` isn't a dependency, so deployment runs outside the repo (CI / Cloudflare git integration). Regen `worker-configuration.d.ts` with `wrangler types` out-of-band after binding/`main` changes.
...
Pages are Astro files under `src/pages/` with `output: "server"` on the `@astrojs/cloudflare` adapter. Each page **fetches its initial data server-side** through the shared data layer (`src/data/api.ts`) using `env` from `cloudflare:workers` and `ctx` from `Astro.locals.cfContext`, renders the first paint, and passes that payload as `initialData` to a React island mounted `client:only="react"` (`@astrojs/react`). The island hydrates, seeds its hook from `initialData`, and takes over interactivity + polling. `AppProviders.tsx` wraps every island with theme context. The `@astrojs/cloudflare` v14 adapter provisions a real KV binding to `astro dev` (via wrangler/miniflare), so local dev exercises the production cache path — there is **no** separate vite dev proxy.

### Routing is split: Astro owns the entry, a custom router owns in-app nav
- **`src/middleware.ts`** (Astro middleware, every request) passes `/` (the global home), comp-prefixed routes, `/api`, and asset paths through; 307-redirects removed/legacy paths (`/fifa.world/**` → `/`, `/<comp>/news|bracket` → `/<comp>`, legacy `/news*` → the default-comp hub).
- **File-based routes**: a top-level `index` (global home), `[comp]/{index,schedule,stats,transactions,odds,teams}`, `[comp]/match/[slug]`, `[comp]/team/[id]`, `[comp]/player/[id]`, `api/[...route]`, `sitemap.xml.ts`. `/<comp>` is the news-first hub and `/<comp>/schedule` the fixtures; `/<comp>/news` 307-redirects to the hub. (`/scorers` 307-redirects to `/stats`; there is no top-level `/player`.)
- **`src/utils/router.ts`** is a thin pure URL builder/reader — `Route` union, `parseRoute`/`pathFor`, and `navigate()` (a real `window.location` assign/replace, **not** pushState — the client router was removed; every view is its own SSR document). **Every route carries its competition** as the first URL segment. `useRouter()` just parses `window.location.pathname` (no subscription — a navigate reloads). Path segments are untrusted — `safeDecode` defends against `URIError`.

### Multi-competition, multi-sport — driven by one registry
`src/competitions.ts` is the **single source of truth** for which competitions the app serves (`eng.1`, `nba`; `DEFAULT_COMPETITION = 'eng.1'`) and their `sport`, ESPN `league` slug, `capabilities` (scorers/lineups/boxscore), `shape` (`tournament` vs `season`), and `leadersSource`. It's pure data + a pure `buildUrl()` — no DOM/React — so it compiles under **both** tsconfigs (app + worker) and every consumer (Astro pages, the worker wrapper, the switcher) builds ESPN URLs from it, so they never drift. `seasonForDate()` handles cross-year season rollover per sport (soccer keys by starting year, rolling in August; NBA keys by ending year, rolling in October). When adding a competition, edit this file.
...
- SSR helpers: `getCompetitionView` (scoreboard + standings + adapter → ready-to-render view), `getLeaderboards` (stats page), `getCompMatchBySlug`, `getMatchSummary`, `getCompNews`, `getTeams`, `getTeamDetail`, `getTransactions`, `getLeagueInjuries`. Astro pages call these directly; islands call same-origin `/api/*`.
...
### One data backend: ESPN
- **ESPN** (public site/core API, no key, CORS-open) provides all sports + news data, cached in KV by the shared layer. (A browser-side live-stream player from third-party aggregators was removed in 2026-07.)
...
`src/leaders.ts` (`assembleLeaders`/`assembleLeaderboards`) is a **pure** aggregator (fetch injected for testing) shared by the worker's `serveLeaders`/`serveLeaderboards` and the SSR `getLeaderboards` (stats page); `useLeaders.ts` reaches it via same-origin `/api/<comp>/leaders`. ESPN's season leaders live on core.api as refs (`$ref`) with no inline names, so it fans out to resolve athlete/team. Competitions declare `leadersSource: 'pipeline'` (eng.1 goals / nba points). The `'scoreboard'` option (aggregating per-team leaders from the scoreboard) is retained on the field for future tournament comps but unused now. `assembleLeaders` throws on a primary-doc failure so serve-stale can cover an outage instead of overwriting a valid leaderboard with an empty one.

### News feed
Per-competition news from ESPN's site.api league feed (`site/v2/.../{league}/news`, genuinely league-scoped — the old global "now" firehose was removed). `src/newsFeed.ts` (`parseNewsFeed` → `NewsItem[]`). Server side: `getCompNews` (api.ts; composes `serve(news)` + parse, empty array on any failure) seeds the `[comp]` news hub; `getAggregatedNews` merges every competition's feed (dedupe by id, newest-first) for the global home. `NewsIsland` + `NewsView` + `useNews.ts` take over client-side (120s visibility-gated poll, scroll-to-load). The `Ticker` island (module-level shared poller via `useTicker`, displays live/upcoming matches in a horizontal strip) is independent of the news feed.

### Data hooks share a pattern
`useCompetition.ts`, `useLeaders.ts`, `useMatchDetail.ts`, `useNews.ts` all follow: **stale-while-revalidate** (seeded from SSR `initialData`, refetch in background), `AbortController` per fetch, and **visibility-gated polling** (paused when tab hidden). Hooks keyed by `comp` (or `eventId`) reset cache + displayed state when the key changes, so one competition's data never flashes on another's tab.
