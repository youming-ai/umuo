...
> AI-curated news desk on Cloudflare Workers. Pull any RSS/JSON feeds, classify with an OpenAI-compatible LLM, store in D1, render as a fast Astro masonry board. Fork, replace the example football config, and you have your own desk.
...
*No ads, no tracking. The football taxonomy in `src/competitions.ts` is just an example — delete it.*

---

## Features

- **15-min cron ingest** — fans out over any RSS / ESPN-style JSON sources, de-duplicates by fingerprint + normalized title, enriches in LLM chunks of 8
...
- **Masonry board** — native CSS multi-column, infinite scroll, per-competition `/[comp]` hubs
- **SEO ready** — `/rss.xml`, `/[comp]/rss.xml`, `/sitemap.xml`, `/sitemap-news.xml` (Google News, last 48h), canonical/og tags, JSON-LD
...
| Taxonomy | `src/competitions.ts` | `FOOTBALL_COMPETITIONS` — keys become `/:comp` routes |
...
| `/:comp` | Per-competition hub (e.g. `/eng.1`) |
...
KV + D1 + Workers free tier covers a personal/small-team desk. LLM is the only variable cost — 31 feeds × ~700 articles/day × ~1 LLM call per 8 articles ≈ 90 calls/day.
...
- **400 on ingest** — check `RSS_HEADERS` / `API_JSON_HEADERS` UA allowlists (ESPN WAF is picky → `curl/8.7.1`).
