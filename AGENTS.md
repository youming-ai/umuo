...
- `worker/entrypoint.ts` — `fetch` (Astro), `scheduled` (cron). `worker/index.ts` is the dispatcher for `/api/*` (explore endpoints) and `/media/*` (upstream image proxy), then falls through to `ASSETS`.
